...
partly the baseline. **[SUPERSEDED BY RUN 21, 2026-09-16: the panics are fixed, the fallback rate
is 0.5-1.8%, and the measurement is 53-26 (67.1%, p=0.0032) with a 20-20 seat control.]** `PanicException` subclasses `BaseException`, so the driver had to catch
...
### Run 15 root-caused defect 1's fault B — and it was Knock Off, not mega evolution at all

**The mega panic was a symptom.** `mega_evolve` panics because the body it is asked to evolve has
lost its stone, and the stone is lost by **Knock Off**, which reads and mutates slot 0 while
stamping the instruction with the real target's slot. Reversing that instruction then *plants slot
0's item on the target's body*. Mega panics on the identical gen 6 arm fell **~15 → 3**.

**The method is the point, because reading the source had already failed three times.** Runs 14 and
15 between them killed three plausible causes with instruments — a slot-indexed `FormeChange`
reversing onto a stranger (0 mismatches), my own translator misaligning species and items (every
root pair correct), and the `Effect::RemoveItem` secondary (fixed on the same evidence, but it moved
no number — see the caveat below). What finally worked was **two instruments, not one**:

1. **the sole mutator** — `State::change_item`, exactly as run 12 instrumented `Side::apply_boost`.
   It proved a wrong write exists and named the victim: `GOTHITELLE NONE -> DIANCITE`, **159 times
   in four battles**, a stone landing on a body that cannot use it.
2. **the constructor** — and this is the step that was missing. The mutator says *that* a bad write
   happened; only the constructor says *which code decided it*. `ChangeItemInstruction::new` is
   generated by the `slotted_instruction!` macro, so **one `#[track_caller]` plus
   `Location::caller()` inside the macro tags every construction site in the engine at once**:

```
31  slot=0 fields=DIANCITE,NONE at src/genx/choice_effects.rs:781:59
 7  slot=1 fields=DIANCITE,NONE at src/genx/choice_effects.rs:781:59
```

The `slot=1` rows carrying **slot 0's item** are the defect, printed with its file and line.

**The defect.** `Choices::KNOCKOFF` (`genx/choice_effects.rs:775`):

```rust
let defender_active = defending_side.get_active();          // slot 0
if defender_active.item_can_be_removed() && … {
    …ChangeItemInstruction::new(def_pos.side, def_pos.slot,  // the REAL slot
                                defender_active.item,        // slot 0's item
                                Items::NONE);
    defender_active.item = Items::NONE;                      // mutates slot 0
}
```

Knocking a slot-1 body therefore **tests slot 0's item**, **clears slot 0 in place**, **names slot 1
in the instruction**, and on reverse **restores slot 0's item onto slot 1**. `THIEF` immediately
below has the same fault on both bodies. Fix: read through `get_active_slot(def_pos.slot)` and
`get_active_slot(act_slot)`. Singles is unaffected — `get_active_slot(0)` is `get_active()`.

| identical 4-battle probe, seed 101, gen 6 | before | after |
|---|---|---|
| stone writes onto a body that cannot use it | **159** (all `GOTHITELLE <- DIANCITE`) | **0** |
| total stone writes seen | 661 | 7123 |
| mega panics, 20-battle arm | ~15 | **3** |

The stone-write count rising tenfold is not noise to explain away — it is the search reaching
deeper once branches stop dying on corrupted state.

**A caveat I am not going to launder: the `Effect::RemoveItem` fix rode along unpriced.**
`generate_instructions.rs:1199` has the identical shape and was fixed earlier in this run on that
reasoning, but its own before/after showed **no change** (159 bad writes before it, 159 after). It is
a correctness fix by inspection matching a shape now proven elsewhere, not a measured one — the
secondary is rare because Knock Off is a *primary* effect, not a secondary. Runs 4 and 5 warn about
exactly this kind of bundling; it is recorded rather than hidden.

**This is a class, not a defect — twenty-five sites (new defect 34).** An audit of
`choice_effects.rs` for "a `get_active()` read paired with a slot-stamped instruction" finds the
same shape in `HEATCRASH`, `KNOCKOFF`, `THIEF`, `METEORBEAM`, `BELLYDRUM`✅, `COUNTER`,
`MIRRORCOAT`, `METALBURST`, `SUPERFANG`, `NIGHTSHADE`, `SEISMICTOSS`, `ENDEAVOR`, `FINALGAMBIT`,
`SUBSTITUTE` and `TRICK`. Only the two the instrument implicated were fixed; **fixing twenty-five
blind is exactly what runs 4 and 5 warn against**. Note what sits in that list: **`BELLYDRUM`**,
which run 14 recorded as the standing lead for defect 2's second cause — the one clamp-bypassing
emit that can move ±12 in a single step. That lead and this class are now plausibly the same
finding, which is a reason to take the audit seriously and still not a measurement.

**Controls, and one of them cannot see this fix.** cargo doubles **216/1 + 46/1** (the two
known-wrong fixtures, no third), singles **220 + 611 + 15** green, corpus **306/316 + 315/316
byte-identical**, stage 0 **19/19**. Knock Off exists in gen 5, so the corpus is not vacuous here
the way it was for `mega_evolve` — but it projects **damage and boosts, never items**, so
byte-identical is a genuine no-regression result and **not** a validation of the fix. Run 5's lesson,
again: check whether the instrument can see the thing before reading anything into its silence. Patch now **1204 lines**, still 12 files — `src/genx/choice_effects.rs`
was already in it from run 7's `reset_boosts` work, so this adds hunks rather than a file.

**What is left on the gen 6 arm**: 3 mega panics, 14 boost panics (defect 2's second cause, still
open), and **9 `unparsed label '<move>-mega,<slot>'` — mine, not the engine's**:
`choice_labels` teaches the label its slot but not its mega suffix, so those decisions fall back
even when the engine is right.

### Run 16 found defect 2's second cause: Belly Drum, exactly where run 14 guessed

Run 14 recorded a lead — gen 6 still threw the ±6 escape with run 12's fix in, and `BELLYDRUM` was
the only clamp-bypassing emit that can move ±12 in one step. Run 15 then found it sitting inside
defect 34's twenty-five-site class. **It is the cause, and this run measured it rather than
inheriting the guess.**

`Choices::BELLYDRUM` (`genx/choice_effects.rs:1273`) had **four** slot-0 accesses against
`act_slot`-stamped instructions:

```rust
let boost_amount = 6 - attacking_side.get_active().attack_boost;   // slot 0's boost
let attacker = attacking_side.get_active();                        // slot 0's body
…BoostInstruction::new(*attacking_side_ref, act_slot, Attack, boost_amount)   // stamps act_slot
attacker.hp -= attacker.maxhp / 2;                                 // mutates slot 0
attacking_side.get_active().attack_boost = 6;                      // sets slot 0
```

A slot-1 Belly Drum therefore computes its amount from **slot 0's** attack boost and applies it to
**slot 1**. With slot 0 sitting at −6 that is `Attack,+12` onto a partner who was at 0, and the
clamp is nowhere in the path. It is invisible to every gen 5 measurement in this document because
**0 of 183 gen 5 doubles teams carry Belly Drum** — the exact sentence run 12 used to dismiss it,
true of a gen 5 pool and false of gen 6, where Azumarill is standard.

**The instrument pair from run 15 transferred directly**, which is the reusable result: the sole
mutator (`State::apply_boost`, printing any write that leaves ±6) plus `#[track_caller]` on the
macro-generated `BoostInstruction::new`. Two sites came back:

```
2  DBG ctor Boost slot=S fields=Attack,-12 at src/state.rs:1819       <- reset_boosts
1  DBG ctor Boost slot=S fields=Attack,6   at src/genx/choice_effects.rs:1286   <- Belly Drum
1  DBG ctor Boost slot=S fields=Attack,12  at src/state.rs:1819       <- reset_boosts
```

**`reset_boosts` is innocent and looks guilty**, which is worth recording because run 7 already
spent a run on that function: it reads and emits for the same slot correctly, and its `±12` is it
zeroing a value Belly Drum had already corrupted. The instrument names whoever *writes*, not
whoever *causes* — the constructor tag is what separates them.

| identical 8-battle probe, seed 101, gen 6 | before | after |
|---|---|---|
| boost writes leaving ±6 | **19** | **0** |
| boost panics | 1 | **0** |

**On the 20-battle arm the ±12 family is gone entirely** — no `Invalid boost value: ±12`, no
`Invalid boost number: ±12`, where the run-15 build threw six of them. What remains is a **different
escape of ±7/−8**, one or two steps past the clamp rather than a runaway, 12 across the arm. That is
a separate site and the same method will find it: the range-check instrument plus the constructor
tag are both written and are recorded here.

**Controls.** cargo doubles **216/1 + 46/1** (the two known-wrong fixtures), singles
**220 + 611 + 15** green, `cargo check` clean for gen1/2/3/4/6, corpus **306/316 + 315/316
byte-identical**, stage 0 **19/19**. As in run 15 the gen 5 corpus cannot see this fix — no Belly
Drum in the pool — so byte-identical is no-regression, not validation.

### Run 17 closed defect 2 on gen 6: the two switch-in abilities that hand-rolled their own boost

Belly Drum left a ±7/−8 residual. It was two more sites, both in `ability_on_switch_in`, and both
the same mistake written twice. **Out-of-range boost writes went 44 → 14 → 0 and the gen 6 arm now
throws no boost panic at all.**

`INTREPIDSWORD` and `DAUNTLESSSHIELD` are byte-for-byte identical bar the stat:

```rust
// no need to check for boost at +6 because we are switching in
attacking_side.get_active().attack_boost += 1;      // slot 0, unclamped, in place
…BoostInstruction::new(*side_ref, owner_slot, Attack, 1)   // stamps owner_slot
```

Two faults, and the comment is the interesting one. **It is a correct piece of reasoning resting on
a false premise**: a body that is switching in genuinely cannot be at +6, so skipping the clamp is
sound — *if the body being mutated is the one switching in*. The slot-0 read is exactly what breaks
that, so a partner sitting at +6 goes to +7 and +8. The clamp and the slot are not two bugs here;
the missing clamp is only reachable *because* of the wrong slot.

`apply_boost_instruction` is the clamping helper this same file already uses two arms above, in
`EMBODYASPECTHEARTHFLAME`, and it takes the slot it mutates — so routing through it fixes both
halves at once and deletes the hand-rolled increment rather than adding a guard to it.

| identical 20-battle gen 6 arm, seed 101 | run 16 | +Intrepid Sword | +Dauntless Shield |
|---|---|---|---|
| boost writes leaving ±6 | 44 | 14 | **0** |
| boost panics on the arm | 12 | — | **0** |

**The residual named its own successor, which is worth recording as method.** After the Intrepid
Sword fix the remaining 14 were *all* Defense, ±1, and symmetric — 7 on slot 0 going `+1 → 7` and 7
on slot 1 going `−1 → −7`. That is run 12's signature exactly: a positive stamped on the wrong body,
with the negatives being its own reversals. The distribution identified Dauntless Shield before any
source was read. **I had seen that site an hour earlier and deliberately left it**, on the rule that
only instrument-implicated sites get fixed; the rule was right, and the evidence arrived one run
later.

**Where the gen 6 arm now stands: 16 fallbacks in 207 decisions (7.7%), down from ~18%** — and
**13 of the 16 are mine, not the engine's**: `unparsed label '<move>-mega,<slot>'`, because
`choice_labels` teaches the label its slot but not its mega suffix. The engine's own share is **3
mega panics** (`KANGASKHAN with EVIOLITE`/`NONE`). The largest remaining item on this board is a
harness bug.

**Controls.** cargo doubles **216/1 + 46/1** (the two known-wrong fixtures), singles
**220 + 611 + 15** (+17 +1) green, `cargo check` clean for gen1/2/3/4/6, corpus **306/316 + 315/316
...
### Run 19 closed defect 1: the residual mega panics were Trick, the class a fourth time

**Defect 1 is closed.** The three surviving `cannot mega evolve KANGASKHAN with …` panics were
never a mega-evolution defect at all — the same conclusion run 15 reached for the first batch, and
by the same mechanism: **something takes the stone off the body before it evolves.** Run 15's
culprit was Knock Off; this one is Trick.

**`PE_DBG_ITEM=1` found it without a hypothesis.** The instruments are a tracked patch now, so this
was one command rather than a rebuild. Filtering the item trace for Kangaskhan showed the stone
moving to bodies that cannot use it, *in pairs that swap back*:

```
166  DBG change_item side=0 slot=0 body=CONKELDURR KANGASKHANITE -> ASSAULTVEST
147  DBG change_item side=0 slot=0 body=CONKELDURR ASSAULTVEST -> KANGASKHANITE
147  DBG change_item side=1 slot=1 body=JIRACHI ASSAULTVEST -> KANGASKHANITE
```

A two-way swap is Trick, and `TRICK | SWITCHEROO` was sitting unfixed in defect 34's list.

**It is the worst shape in that class so far, because it is a swap.** All four accesses read slot
0 while both instructions stamp the real slots:

```rust
let defender_has_sub = defending_side.get_active()…      // guard, slot 0
let attacker = attacking_side.get_active();              // slot 0
let defender = defending_side.get_active();              // slot 0
…ChangeItemInstruction::new(*attacking_side_ref, act_slot,      attacker_item, defender_item)
…ChangeItemInstruction::new(def_pos.side,        def_pos.slot,  defender_item, attacker_item)
attacker.item = defender_item;                           // mutates slot 0
defender.item = attacker_item;                           // mutates slot 0
```

So a slot-1 Trick reads slot 0's item on **both** sides, swaps them **in place on slot 0**, and
emits instructions naming the real slots carrying slot 0's values — planting a stranger's item on
each and stripping two bodies that were never involved. **The guard is wrong too**: whether the
swap happens at all was decided from the wrong bodies' items and Substitute.

| identical 40-battle gen 6 arm, seed 101 | control | fixed |
|---|---|---|
| `cannot mega evolve` panics | **3** | **0** |
| decisions | 385 | 411 |

**A harness defect this run exposed, and it is not a regression — verified rather than assumed.**
The 40-battle arm reports **37 `state build TypeError`** in *both* columns, which is new only
because **the gen 6 arm had never been run past 20 battles**. Checking out the pre-run-18 harness
and re-running gave **37 as well**, so run 18's mega work did not introduce it. The cause is a
team-import artifact: `p.moveSlots` contains an entry whose id is a **concatenation** —
`tailwindsteelwing`, `psyshockhiddenpowerground…` — which is `toID()` applied to an *array* of move
names, with `pp: null` following from it. Recorded as **defect 35**, mine not the engine's.

**And the counter that hid it is fixed.** It read `fallback: state build TypeError` — a class name
and nothing else, which is run 9's "a count of a summary is not a count of the thing" and cost a
diagnostic re-run. It now carries the message.

**Controls.** cargo doubles **216/1 + 46/1** (the two known-wrong fixtures), singles
**220 + 611 + 15** (+17) green, `cargo check` clean for gen1/2/3/4/6, corpus **306/316 + 315/316
byte-identical**, stage 0 **19/19**.

**Defect 34's class is now 4 of 25 fixed** — Knock Off, Thief, Belly Drum, Trick — and **all four
...
### Run 21: the gen 6 measurement, blocked since 2026-09-13, finally runs — 53-26

The gen 6 section above ends "the path to a meaningful number now runs through fixing these panics
first". Twelve runs later the panics are fixed and the number exists. **Three arms, one seed, run
sequentially** because the MCTS budget is wall-clock; `gen6doublesou`, 338 complete six-mon teams,
40 battles each at 300 ms.

| arm | result | | p (two-sided binomial) |
|---|---|---|---|
| seat A — mcts as p1 | **28-12** | 70.0% | 0.017 |
| seat B — mcts as p2 | **25-14** (+1 turn limit) | 64.1% | 0.108 |
| **pooled** | **53-26** | **67.1%** | **0.0032** |
| control — greedy vs greedy | 20-20 | 50.0% | 1.0 |

**The control is the best this study has had.** Gen 5's seat control was a lopsided 23-17; gen 6's
is **exactly 20-20**, so the seat confers nothing and the pooled 67.1% cannot be a seat artefact.
Seat symmetry is clean too (Fisher p=0.64 between the two seats). The weakest figure is the honest
one: **mcts-p1 against greedy-p1 in the same seat is Fisher p=0.11** — suggestive, not conclusive,
exactly as gen 5's 0.053 was.

**And this arm is almost entirely the search, which no previous gen 6 attempt could say**:
fallbacks are **2 of 438 (0.5%)** in seat A and **8 of 457 (1.8%)** in seat B, against the ~18% of
run 17 and the wholesale panicking of 2026-09-13. A score from this arm means something because
the search actually took the turns.

**It is lower than gen 5's 80.0%, and that should not be explained away.** Both are real teams, the
same harness and the same referee. Gen 6 is a format with megas, a larger legal pool and more
speed control, so a greedy baseline may simply be harder to beat there; or the two tiers differ in
how punishing a greedy policy is. **Nothing here distinguishes those**, and the study should not
claim it does.

**Two new fallback causes, both small and both worth naming:**

- **`get_two_actives called with the same position` ×2 — that assert is defect 3, marked FIXED in
  run 10.** Gen 6 has never exercised it. It is either a second path into the same assert or a
  regression, and **this document should not guess which**: run 10's fix re-derived the target
  after Encore substituted the move, and nothing in this run implicates Encore. Recorded as
  **defect 3 reopened**, to be instrumented rather than reasoned about.
- **`volatile stockpile` ×7 — mine.** `pe_doubles_corpus.py` has no mapping for `stockpile`, so
  the translator refuses the position. Run 11's lesson applies directly: check that a mapped
  volatile is actually passed, and that an unmapped one is unmappable rather than merely absent.
...
| 36 | open — **mine**, `tools/pe_doubles_corpus.py` | **`stockpile` has no volatile mapping**, so the translator refuses the position outright: 7 of 457 decisions (1.5%) in run 21's seat B. Run 11's lesson applies — check that a mapped volatile is actually PASSED, and that an unmapped one is genuinely unmappable rather than merely absent from the table |
...
`patches/poke_engine_doubles_spread_per_target.patch` (1323 lines, 12 files; reverses cleanly).
