...
PBAI::SwitchHandler.add_out do |score,ai,battler,target|
	next unless target.moves.any? {|move| move.damagingMove? && move.type == :COSMIC}
	target_moves = target.moves
	for i in target_moves
		next if target_moves == nil
	...
	if has_move
			score += 3
			PBAI.log_switch_out(3,"Cosmic type matchup")
...
PBAI::SwitchHandler.add_out do |score,ai,battler,target|
	if battler.trapped?
    score -= 20
    PBAI.log_switch_out(-20,"Trapped so don't try to score")
  end
...
PBAI::SwitchHandler.add_out do |score,ai,battler,target|
	if battler.effects[PBEffects::Yawn] == 1
		score += 4
		PBAI.log_switch_out(4,"Try to switch due to being yawned")
	end
	next score
end
...
PBAI::SwitchHandler.add_out do |score,ai,battler,target|
	if battler.effects[PBEffects::SaltCure]
		prio = battler.pbHasType?(:WATER) || battler.pbHasType?(:STEEL)
		add = prio ? 6 : 3
		score += add
		PBAI.log_switch_out(add,"Try to switch due to being salt cured")
	end
	next score
end
...
PBAI::SwitchHandler.add_out do |score,ai,battler,target|
	pos = ai.battle.positions[battler.index]
	party = ai.battle.pbParty(battler.index)
	tspikes = battler.own_side.effects[PBEffects::ToxicSpikes] == 0 ? 0 : battler.own_side.effects[PBEffects::ToxicSpikes]
	if tspikes > 0
...
PBAI::SwitchHandler.add_out do |score,ai,battler,target|
    ...
    score += 20
    PBAI.log_switch_out(20,"Preventing Struggle")
  end
	next score
end
...
PBAI::SwitchHandler.add_out do |score,ai,battler,target|
    ...
    score += 20
    PBAI.log_switch_out(20,"Dodging Perish Song")
  end
	next score
end
...
PBAI::SwitchHandler.add_out do |score,ai,battler,target|
	if battler.effects[PBEffects::Unburden] && !battler.item && battler.hasActiveAbility?(:UNBURDEN)
    score -= 10
...
# Don't switch if you are low
PBAI::SwitchHandler.add_out do |score,ai,battler,target|
	if battler.hp <= battler.totalhp/2
		score -= 20
		PBAI.log_switch_out(-20,"Not switching below half health")
	end
	next score
end
...
PBAI::SwitchHandler.add_out do |score,ai,battler,target|
	...
	party.each do |pkmn|
		...
		pk = ai.pokemon_to_projection(pkmn)
		moves = 0
		best = 0
		target.moves.each do |move|
			dmg = pk.get_potential_move_damage(target, move)
			moves += 1 if move.damagingMove? && dmg >= mon.hp/2
			best += 1 if move.damagingMove? && dmg < mon.hp/2
		end
		PBAI.log_switch_out(0,"Good moves against #{pkmn.name}: #{moves}")
		if moves == 0 && best > 0
			add += 1
		end
	end
	if add > 0
		score += add
		PBAI.log_switch_out(add,"for having #{add} good switch ins")
	end
	if add == 0
		score -= 3
		PBAI.log_switch_out(-3,"for having no good switch ins")
	end
	PBAI.log_switch_out(0,"Good switch ins: #{add}")
	next score
end
