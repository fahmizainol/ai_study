...
| `tools/boss_studio.py` | local UI (127.0.0.1:8731): the knobs, all 27 rosters live (nine gyms + 18 named trainers), the §6.8 candidates panel, and Install, which writes both team files, regenerates the registry and injects it into the game bundle — all four replaced together or none. Export and Install both write a **companion preset** named after the gym team file (`teams_bosses_gyms` → `generated/studio_presets/teams_bosses_gyms.json`) holding all 27 fights frozen, and stamp `design.preset` into every record — `design` is safe for this because neither `emit_registry.py` nor `validate_team.py` reads it. Loading a team JSON follows that tie and replays the teams exactly; a file with no companion is reverse-engineered from the knobs instead, and the UI says which of the two happened. The companion is frozen even when the live session is not, so handing someone an exact artifact does not pin the cards you are still editing. A fight can also be **frozen** directly (`FROZEN` in a preset, keyed `g<n>`/`t<slot>`): it then ships the exact team it was frozen at and the generator is not consulted for it, so knobs, card overrides and a moved PBS/Smogon corpus all stop reaching it. That is what lets a committed preset rebuild the same teams on another machine, which an unfrozen one cannot promise |
